...
- TreeNode 节点本身只存储 val, 对应的字符是通过在父节点的 children 数组中的索引确定的 （用树枝存储 key, 用节点存储 value）
...
```
- 但其实呢，next 节点并不需要递归来返回；递归可以返回反转之后的头结点
```java
public ListNode reverseList(ListNode head) {
    // 递归终点
...
- 但是在遍历链表的时候，无法得知当前位置的下标，只知道自己遍历了多少个元素
- 因此 reverseBetween --> reverseN
```java
class Solution {
    private ListNode newHead = null;
    public ListNode reverseBetween(ListNode head, int left, int right) {
        // 反转 left 到 right 位置的链表
        // 但是在遍历链表时，无法得知当前的下标，只知道便利了多少个元素
        // 因此 reverseBetween --> reverseN
        ...
        ListNode newTail = reverseN(prevNode.next, right-left+1, 1);
        prevNode.next = newHead;
        newTail.next = nextNode;
        return dummyHead.next;
    }

    private ListNode reverseN(ListNode node, int N, int n) {
        if (n == N) {
            newHead = node;
            return node;
        } 
        ListNode prev = reverseN(node.next, N, n+1);
        prev.next = node;
        return node;  
    }
}
...
#### 234. 回文链表
##### 翻转链表，然后同时遍历的思路
- 需要注意，不能破坏原链表的结构；因此需要拷贝节点，而不是直接在原节点上操作
```java
class Solution {
    public boolean isPalindrome(ListNode head) {
        if (head == null) return true;
        ListNode newHead = reverseList(new ListNode(head.val, head.next));
        while (head != null) {
            if (newHead == null) {
                return false;
            }
    ...
    private ListNode reverseList(ListNode node) {
        // 不再返回下一个节点，只返回 reverse 之后的头结点？
        // 每次递归调用中，就完成这个节点的 reverse
        if (node == null || node.next == null) {
            return node;
        }
        ListNode nextNode = new ListNode(node.next.val, node.next.next);
...
##### 一遍递归，直接判断的思路
- 后序遍历就能得到反过来遍历链表的结果（其实和翻转链表一样）
- 后序: 递归函数的出口处
- 在后序位置，我们和正常顺序的链表做个对比
```java
ListNode left;
public boolean isPalindrome(ListNode head) {
    left = head;
...
class Solution {
    public int removeElement(int[] nums, int val) {
        ...
        int slow = nums.length-1, fast = nums.length-1; // 反向遍历
        while (fast >= 0) {
            if (nums[fast] == val) {
                nums[fast] = nums[slow]; // 不需要交换，把不是 val 的元素移到前面即可
                fast--;
                slow--;
            } else {
                fast--;
...
#### 差分数组
- 主要目的：快速进行区间增减操作
- 主要还是为了降低时间复杂度吧（本来是一整个区间的操作，现在只需要对区间的头尾元素操作一下就好）
- 差分数组：存储相邻元素之差
##### 1109. 航班预订统计（已复习，注意差分数组中也可以有为负数的元素）
- 对于差分数组，定义了三个函数
    - 构造函数，初始化差分数组
...
class Solution {
    ...
    private void traverse() {
        ...
        while (true) {
            int[] directs = directions[directionIndex % directions.length];
            // 越界检查 和 已遍历过检查
            if (row + directs[0] >= rowLen || row + directs[0] < 0 || col + directs[1] >= colLen || col + directs[1] < 0 || visited[row + directs[0]][col+directs[1]])  {
                directionIndex++;
                if (check_completed(row, col)) break;
            } else {
                row += directs[0];
                col += directs[1];
                result.add(matrix[row][col]);
    ...
    private boolean check_completed(int rowIndex, int colIndex) {
        for (int[] directs: directions) {
            int rowUpdated = rowIndex + directs[0];
            int colUpdated = colIndex + directs[1];
            if (rowUpdated >= rowLen || rowUpdated < 0 || colUpdated >= colLen || colUpdated < 0) {// 越界
                continue;
            }
            if (visited[rowUpdated][colUpdated]) {
                continue;
            }
            return false;
        }
...
class Solution {
    ...
    private int cols;
    private boolean[][] visited;
    private boolean valid_index(int r, int c) {
    ...
    public int[][] spiralMatrixIII(int rows, int cols, int rStart, int cStart) {
        ...
        this.cols = cols;
        int total = rows * cols;
        int[][] results = new int[total][2];
        int resultsIndex = 0;
        int down_bound = rStart, up_bound = rStart;
        int left_bound = cStart, right_bound = cStart;
        visited = new boolean[rows][cols];

        while (resultsIndex < total) {
            // 向右
            while (cStart <= right_bound) {
                if (valid_index(rStart, cStart)) {
                    visited[rStart][cStart] = true;
                    results[resultsIndex++] = new int[] {rStart, cStart};
                }
                cStart++;
            }
            right_bound++;
...
```
#### 543. 二叉树的最大直径(已复习，注意最大直径不一定是左子树最深处到右子树最深处)
...
#### 116. 填充每个节点的下一个右侧节点指针(递归思路想不到，需要再复习)
- 第一种思路是层序遍历，把每一层的元素都连起来
- 第二种思路是遍历二叉树
    - 普通的遍历，无法处理左子树的右孩子，和右子树的左孩子之间的联系
    - 其实就是第三层中有一种情况处理不了（(left.right 和 right.left) 之间的边）
    - 因此遍历思路成了一个三叉树的遍历: 所有需要连 next 边的情况: [(left.left, left.right), (left.right, right.left), (right.left, right.right)]
```java
class Solution {
...
```
#### 114. 二叉树展开为链表（分解问题 + 递归，需要再复习）
- 用分解问题 + 递归的思路
    - 先把左右子树递归地处理好；
    - 然后把链表组合起来，按照根左右的顺序
        - 需要找到左子树的最右节点，其 next 节点指向右子树
- 要考虑的坑: 如果左子树为空，记得直接让根节点的 next 指向右子树
```java
...
##### 315.计算右侧小于当前元素的个数（已复习）
- 在归并排序的基础上，在 merge 函数中夹杂私货，计算右边有多少元素更小
- 每次 merge 的时候，对于左半边数组，可以计算右半边数组有多少元素更小
- merge 被递归调用，在入口处就能获得每个元素右边有多少元素更小
```java
class Solution {
...
##### 207 课程表(已复习，有新的笔记)
- 注意点:
    - DFS(回溯法) 的框架要记住，访问完当前节点之后，要从 onPath 中把节点移除（也就是撤销选择）
    - 声明一个数组时，必须确定数组元素的类型（不能够是泛型）；因此 array of LinkedList 的写法如下
```java
...
```
###### 新的思路
- 最小生成树最后总共要有 N-1 条边，以此作为终止条件
- 初始我们选定 0 号节点，那么就把 0 号节点周围的边都加入队列中
- 每次队列中出来一条边，如果这个边指向的另一个节点没有访问过，就把他加入最小生成树中; 并把新节点的邻接边加入队列
- 如何保证"最小"?
    - 这个队列是一个优先级队列，每次出队距离最小的那条边
    - 将邻接边加入队列时，就可以判断另一个顶点是否访问过
```java
class Solution {
    private int n;
    private int[][] points;
    ...
    public int minCostConnectPoints(int[][] points) {
        ...
        while (edgeCount < n-1) {
            ...
            if (!visited[index]) {
                ...
                edgeCount += 1;
                add_neighbors(index, pq);
            }
        }
        return minimumCost;
    }

    private void add_neighbors(int point_index, PriorityQueue pq) {
        for (int i=0; i<n; i++) {
            if (i!=point_index && !visited[i]) {
                pq.offer(new int[]{
                    i, manhattan_distance(points[point_index], points[i])
                });
            }
        }
    }

    private int manhattan_distance(int[] point1, int[] point2) {
        int x1 = point1[0];
        int y1 = point1[1];
        int x2 = point2[0];
        int y2 = point2[1];
...
###### 二维数组做法
- 其实三维数组中，最后一维的 0/1 可以并入第二维中，我感觉代码写起来更加流畅（特别是初始值设置这边）
- 同时，我觉得把状态定义为`第 k 笔交易买入后` 和 `第 k 笔交易卖出后` 更好理解
```java
class Solution {
    public int maxProfit(int k, int[] prices) {
        int n = prices.length;
        if (n <= 1) return 0;
        int[][] dp = new int[n][2*k+1]; // 2*m-1: 第 m 笔交易买入; 2*m: 第 m 笔交易卖出
        dp[0][0] = 0;
        dp[0][1] = -prices[0];
        dp[0][2] = 0;
        for (int j=3; j<2*k+1; j++) { // 第一天不可能交易多次
            dp[0][j] = -prices[0];
        }
        for (int i=1; i<n; i++) {
            dp[i][0] = 0;
        ...
        for (int j=0; j<2*k+1; j++) {
            maxProfit = Math.max(dp[n-1][j], maxProfit);
        }
        return maxProfit;
    }
}
...
- 递推公式:
    - 如果 dp[i-1] < 0, 那么不如从当前元素开始重新算一个子数组
    - dp[i-1] >= 0, 那就把当前元素加入子数组
- 遍历所有的 dp, 取一个最大值
```java
class Solution {
...
```
##### 1143. 最长公共子序列
- dp[i][j]: 考虑到 text1 的 i-1 处， text2 = j-1 处，最大公共子序列的长度
```java
class Solution {
        ...
        for (int i=1; i<n; i++) {
            for (int j=1; j<m; j++) {
                if (text1.charAt(i-1) == text2.charAt(j-1)) {
                    ...
                    dp[i][j] = Math.max(
                        dp[i-1][j], dp[i][j-1]
                    ); // 为什么没有 dp[i-1][j-1]? 因为dp[i-1][j] >= dp[i-1][j-1] 必然成立
                }
            }
        }
        return dp[n-1][m-1];
...
#### 背包类型问题
##### 0-1 背包问题: 物品要么装进包里，要么不装，不能切成两块
- 状态: 背包容量，可选择的物品
- 选择: 物品是否装进背包
- 数组定义: dp[i][w]: 对于前 i 件物品，在背包容量为 w 的情况下，能装下的最大价值
- 状态转移:dp[i][w] = max(dp[i-1][w], val[i-1] + dp[i-1][w - wt[i-1]]), 后者就是把当前物品装入背包
##### 子集背包问题: 416.分割等和子集
- 首先拿到数组的 sum, 问题就变成了能否使得背包容量恰好是 sum/2
- 选择仍然是物品是否装进背包
- dp[i][w]: true/false: 前 i 件物品, 是否恰好能选择总重为 w 的装法
- dp[i][w] = {dp[i-1][w], dp[i-1][w-val[i]]},  true 优先，否则 false
    - 注意要检查背包是否装得下
```java
class Solution {
    public boolean canPartition(int[] nums) {
        ...
        if (sum % 2 == 1) return false; // 奇数，肯定不行
        int target = sum / 2;
        ...
        for (int i=1; i<nums.length; i++) {
            for (int j=1; j<=target; j++) {
                if (dp[i-1][j]) dp[i][j] = true; // 不装东西
                else if (j-nums[i] >= 0 && dp[i-1][j-nums[i]]) dp[i][j] = true; // 把 nums[i] 装进来
                else dp[i][j] = false;
            }
        }
...
- 区别在于物品可以被装入多次
- 状态和选择不变
- 数组定义: dp[i][w]: 使用前 i 件物品，能凑出重量为 w 的组合数
- dp[i][w]
    - 如果不使用这第 i 件物品: dp[i-1][w]
    - 使用第 i 件物品: dp[i][w-val[i]]; **注意这里下标是 i, 因为物品可以使用多次**
    - dp[i][w] = 两种选择之和
```java
class Solution {
    public int change(int amount, int[] coins) {
        int[][] dp = new int[coins.length+1][amount+1]; // 统一把下标为 0 的地方作为 padding
        // 什么都不做，就能够凑出总金额为 0
        ...
        for (int i=1; i<=coins.length; i++) {
            for (int j=1; j<=amount; j++) {
                dp[i][j] = dp[i-1][j]; // 第i个硬币不装入背包
                if (j-coins[i-1] >= 0) { // 注意 i 和 coins 的下标之间有一位便宜
...
#### 基于有限状态机的 KMP 匹配算法
- 所解决的问题: txt 为文本串; pat 为模式串；目标是从 txt 中找到和 pat 对应的子串
- 暴力解法
```java
// 暴力匹配（伪码）
...
```
- 暴力解法存在的问题：每次遇到不匹配的字符，会同时回退 txt 和 pat 的下标，然后继续遍历匹配
    - 可以不回退 txt, 只回退 pat 的下标
    - pat 的下标不一定要回到 0, 将 pat 视作一个状态机，只要回退到一个等价状态即可
- 状态机示例
<img src="图片/KMP-DFA.png">

- 那么怎么把状态机表示出来呢？
    - 一个 dp[][] 数组；第一维是 pat 的下标，第二维是遇到的字符，取值是要进入的状态
    - 在搜索过程中，dp 数组决定下一步的状态
- 看代码注释！
##### 02-22 结合自动机的笔记
- 对比暴力匹配算法
    - 首先，如果遇到匹配失败的情况，文本串 txt 不要回退，模式串 pat 可以回退
    - 因为文本串没有回退，因此模式串应该回退到一个等价位置；等价位置的由下文 DFA 分析
    - 如果正确匹配，那么文本串和模式串都前进一步
- 什么叫做等价位置（等价状态/回退状态）
    - 如果模式串没有重复字符，那么就没有这个状态
    - 如果模式串中有重复的字符，把自动机画出来，可以发现当不匹配的时候，不一定要回到起点；这就是 KMP 算法的初衷，（匹配失败时）回退到一个等价的，距离起始状态最远的状态，而不是起始状态
- 自动机的定义
    - 如果模式串的长度为 n, 则自动机有 n+1 个状态
    - 每匹配模式串中的一个字符，则进入自动机的下一个状态
- 实现
    - 用一个 dp[][] 数组来表示自动机
        - 第一维是当前所处的专题该
        - 第二维是接收的字符
...
class Solution {
    ...
    private class KMP {
        private int[][] dp; // 第一维: 字符串的下标；第二维: 字符串该下标处的值；取值: 自动机的状态
        private String pat; // 模式串，匹配目标
        ...
        private void construct_dp() {
            dp = new int[this.pat.length()][256]; // 256 对应所有的 ascii 字符
            int back_status = 0; // 回退状态；或者说是当前状态的等价状态（位于前面的）
            dp[0][this.pat.charAt(0)] = 1; // base case
            for (int i=1; i<this.pat.length(); i++) {// 此时的回退状态为 0， 即回到起点
                for (char j=0; j<256; j++) {
                    dp[i][j] = dp[back_status][j]; // 如果字符不匹配: 则回退到对应状态, 然后处理当前字符
                }
                dp[i][this.pat.charAt(i)] = i+1; // 字符匹配: 进入下一个状态
                
                // 更新回退状态
                back_status = dp[back_status][this.pat.charAt(i)];
            }
            this.dp = dp;
        }

        public int search(String txt) {
            // 搜索的过程: 文本串永远不回退，每次只回退模式串（pat）
            int N = txt.length();
            int M = this.pat.length();
            int j=0;
            for (int i=0; i<N; i++) {
                j = dp[j][txt.charAt(i)]; // dp 数组存放所处的状态；可能是进入下一个状态（i+1）, 有可能回退到之前的状态
                if (j == M) return i-M+1; // 进入自动机的接收态
            }
            return -1; // 没有找到匹配项
        }
    }
...
##### 2023-01-19 新的做法
- 按照顺序遍历数组
- 对于每个元素，存在加入 subset 和 不加入这两种选择
- 仍然采用回溯法的框架，遍历所有节点（及这两种选择）
```java
class Solution {
    List<List<Integer>> res = new LinkedList<>();
    int[] nums;
